...
def masterSecret(secret, random1, random2):
    ...
    @see: http://msdn.microsoft.com/en-us/library/cc241992.aspx
    """
    return saltedHash("A", secret, random1, random2) + saltedHash("BB", secret, random1, random2) + saltedHash("CCC",
                                                                                                               secret,
                                                                                                               random1,
                                                                                                               random2)
...
def sessionKeyBlob(secret, random1, random2):
    ...
    """
    return saltedHash("X", secret, random1, random2) + saltedHash("YY", secret, random1, random2) + saltedHash("ZZZ",
                                                                                                               secret,
                                                                                                               random1,
                                                                                                               random2)
...
class RDPInfo(CompositeType):
    ...
    def __init__(self, extendedInfoConditional=PerfFlag.PERF_DISABLE_WALLPAPER):
        ...
        # support flag
        self.flag = UInt32Le(
            InfoFlag.INFO_MOUSE | InfoFlag.INFO_UNICODE | InfoFlag.INFO_LOGONNOTIFY | InfoFlag.INFO_LOGONERRORS | InfoFlag.INFO_DISABLECTRLALTDEL | InfoFlag.INFO_ENABLEWINDOWSKEY)
...
class RDPExtendedInfo(CompositeType):
    ...
    def __init__(self):
        ...
        self.performanceFlags = UInt32Le(
            PerfFlag.PERF_DISABLE_WALLPAPER | PerfFlag.PERF_DISABLE_MENUANIMATIONS | PerfFlag.PERF_DISABLE_CURSOR_SHADOW | PerfFlag.PERF_DISABLE_THEMING)
...
class SecLayer(LayerAutomata, IStreamSender, tpkt.IFastPathListener, tpkt.IFastPathSender, mcs.IGCCConfig):
    ...
    def __init__(self, presentation):
        ...
        # credentials
        self._info = RDPInfo(extendedInfoConditional=lambda: (
        self.getGCCServerSettings().SC_CORE.rdpVersion.value == gcc.Version.RDP_VERSION_5_PLUS))
    ...
    def readEncryptedPayload(self, s, saltedMacGeneration):
        ...
        if saltedMacGeneration and macSaltedData(self._macKey, decrypted, self._nbDecryptedPacket)[
                                   :8] != signature.value:
            raise InvalidExpectedDataException("bad signature")
    ...
    def writeEncryptedPayload(self, data, saltedMacGeneration):
        ...
        if saltedMacGeneration:
            return (String(macSaltedData(self._macKey, s.getvalue(), self._nbEncryptedPacket - 1)[:8]),
